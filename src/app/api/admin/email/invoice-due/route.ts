...
const invoiceDueSchema = z.object({ invoice_id: uuidSchema });

export async function POST(request: Request) {
  const limited = rateLimitOrNull(request, 'email-invoice', ADMIN_EMAIL_LIMIT);
  if (limited) return limited;

  const auth = await requireAdmin();
  if (isAuthError(auth)) return auth;

  const body = await parseBody(request, invoiceDueSchema);
