...
Two naming systems exist — don't confuse them:

- **Webflow semantic** (in `tokens.ts`): `--_color---text--primary`
- **Figma single-dash** (in component CSS + `figma-tokens.css`): `--text-primary`, `--label-tiny`

### CSS token source (CRITICAL)

`globals.css` imports `figma-tokens.css` — this is auto-generated by Style Dictionary (`npm run build:sd-figma` in BDS). **Never manually edit token CSS files.** If a CSS variable is missing, the fix is in BDS's Style Dictionary pipeline, not in a manual CSS file.

```css
/* src/app/globals.css */
@import '../../brik-bds/tokens/fonts.css';
@import '../../brik-bds/tokens/figma-tokens.css';  /* auto-generated, never edit */
```
